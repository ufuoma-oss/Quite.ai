...
// Quitee Logo Component
const QuiteeLogo = ({ size = 48 }: { size?: number }) => (
...
const SignIn: React.FC<SignInProps> = ({ onNavigate }) => {
  ...
  const performLogin = () => {
    setIsLoading(true);
    setTimeout(() => {
      createSession(email || 'user@quitee.app');
      setIsLoading(false);
  ...
  return (
    <section className="relative h-screen w-full flex flex-col items-center justify-center bg-white overflow-hidden touch-none">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[300px] h-[300px] bg-emerald-100 rounded-full blur-[80px]"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-[300px] h-[300px] bg-teal-100 rounded-full blur-[80px]"></div>
      </div>
        ...
        >
          ...
          <div className="text-center mb-5">
            ...
            <p className="text-xs text-neutral-500">
              {isSignUp ? 'Start breaking habits with Quitee today.' : 'Sign in to continue your journey.'}
            </p>
          </div>
          ...
          >
            {isLoading ? (
              <div className="w-4 h-4 border-2 border-neutral-300 border-t-emerald-500 rounded-full animate-spin"></div>
            ) : (
              <>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="space-y-1">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Mail size={16} className="text-neutral-400 group-focus-within:text-emerald-500 transition-colors" />
                </div>
                <input
                  type="email"
                  required
                  ...
                  className="block w-full pl-9 pr-3 py-2.5 border border-neutral-200 rounded-xl text-black placeholder-neutral-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm bg-white"
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center ml-1">
                ...
                {!isSignUp && (
                  <a href="#" className="text-[10px] font-medium text-neutral-500 hover:text-emerald-600">Forgot?</a>
                )}
              </div>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Lock size={16} className="text-neutral-400 group-focus-within:text-emerald-500 transition-colors" />
                </div>
                <input
                  ...
                  className="block w-full pl-9 pr-9 py-2.5 border border-neutral-200 rounded-xl text-black placeholder-neutral-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all text-sm bg-white"
                  placeholder="••••••••"
                />
                <button
                  type="button"
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.99 }}
              className="w-full bg-emerald-500 text-white font-medium py-3 px-4 rounded-xl hover:bg-emerald-600 transition-all transform flex items-center justify-center gap-2 mt-2 cursor-pointer text-sm"
            >
              {isLoading ? (
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
              ) : (
                <>
                  {isSignUp ? 'Get Started' : 'Sign In'} <ArrowRight size={16} />
                </>
              )}
            </motion.button>
          </form>
            ...
            <div className="text-xs text-neutral-500">
              {isSignUp ? (
                <>
                  ...
                  <button onClick={() => setIsSignUp(false)} className="text-emerald-600 font-medium hover:underline">
                    Sign in
                  </button>
                </>
              ) : (
                <>
                  ...
                  <button onClick={() => setIsSignUp(true)} className="text-emerald-600 font-medium hover:underline">
                    Sign up
                  </button>
                </>
              )}
